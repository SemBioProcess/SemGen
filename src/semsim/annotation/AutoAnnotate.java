...
public class AutoAnnotate {
	...
	public static SemSimModel autoAnnotateWithOPB(SemSimModel semsimmodel, SemSimLibrary semsimlib, String cfgreadpath) {		
		...
		fundamentalBaseUnits = SemSimUtil.getAllUnitsAsFundamentalBaseUnits(semsimmodel, cfgreadpath);
		
		// If units present, set up physical property connected to each data structure
		for(DataStructure ds : semsimmodel.getAssociatedDataStructures()){
			
			if(ds.hasUnits()){
				PhysicalPropertyInComposite pp = semsimlib.getOPBAnnotationFromPhysicalUnit(ds);			
		...
		for(DataStructure camount : candidateamounts){
			...
			if(hasinitval && !camount.isDiscrete() 
				...
				camount.setAssociatedPhysicalProperty(semsimlib.getOPBAnnotationFromPhysicalUnit(camount));
		...
		for(DataStructure camount : temp){
			...
			for(DataStructure newcamount : camount.getDownstreamDataStructures(unconfirmedamounts, null)){
				...
				if (!newcamount.hasPhysicalProperty()) {
					newcamount.setAssociatedPhysicalProperty(semsimlib.getOPBAnnotationFromPhysicalUnit(newcamount));
		...
		for(DataStructure cforce : candidateforces){
			...
			if((cforce.hasStartValue() || annotate) && !cforce.isDiscrete() 
				...
				cforce.setAssociatedPhysicalProperty(semsimlib.getOPBAnnotationFromPhysicalUnit(cforce));
		...
		for(DataStructure cforce : temp){
			...
			for(DataStructure newcforce : cforce.getDownstreamDataStructures(unconfirmedforces, null)){
				...
				if(!newcforce.hasPhysicalProperty()){
					newcforce.setAssociatedPhysicalProperty(semsimlib.getOPBAnnotationFromPhysicalUnit(newcforce));
		...
		for(DataStructure cflow : candidateflows){
			...
			if((cflow.hasStartValue() || annotate || cflow.getName().contains(":")) && !cflow.isDiscrete()
				...
				cflow.setAssociatedPhysicalProperty(semsimlib.getOPBAnnotationFromPhysicalUnit(cflow));
		...
		for(DataStructure cflow : temp){
			...
			for(DataStructure newcflow : cflow.getDownstreamDataStructures(unconfirmedflows, null)){
				...
				if(!newcflow.hasPhysicalProperty()){
					newcflow.setAssociatedPhysicalProperty(semsimlib.getOPBAnnotationFromPhysicalUnit(newcflow));
