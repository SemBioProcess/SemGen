...
//		TODO: check that triples with SemSim namespace-based predicates are being output in 
//		accordance with 1.2 spec
		...
		// Replace old DCTERMS namespace with new one used in OMEX metadata spec 1.2
...
//			TODO: problem is that http://www.bhi.washington.edu/SemSim#legacyCodeURI
//				and http://www.bhi.washington.edu/SemSim#SemSimVersion
//					aren't getting renamed
			...
			for(OWLEntity entity : ont.getSignature()) {
				...
				if(oldiri.toString().startsWith("http://www.bhi.washington.edu/SemSim#"))
					manager.applyChanges(renamer.changeIRI(oldiri, 
							IRI.create(RDFNamespace.SEMSIM.getNamespaceAsString() + oldiri.getFragment())));
			}
		...
		// Rename dc properties if old dc namespace is used
		for(OWLEntity entity: ont.getSignature()) {
			IRI oldiri = entity.getIRI();

			if(oldiri.toString().startsWith("https://dublincore.org/specifications/dublin-core/dcmi-terms/"))
				manager.applyChanges(renamer.changeIRI(oldiri, IRI.create(RDFNamespace.DCTERMS.getNamespaceAsString() + oldiri.getFragment())));
		}
		
		// Renamer doesn't seem to work with ontology-level annotations, so we replace each of them that uses
		// an old SemSim namespace. This is also done for the old DC namespace.
		for(OWLAnnotation oldann : ont.getAnnotations()) {
			
			OWLAnnotationProperty oldannprop = oldann.getProperty();
			OWLAnnotationProperty newannprop = null;
			
			OWLAnnotationValue annval = oldann.getValue();

			if(oldannprop.getIRI().toString().startsWith("http://www.bhi.washington.edu/SemSim#")){
				newannprop = factory.getOWLAnnotationProperty(
						IRI.create(RDFNamespace.SEMSIM.getNamespaceAsString() + oldannprop.getIRI().getFragment()));
			}
			else if(oldannprop.getIRI().toString().startsWith("https://dublincore.org/specifications/dublin-core/dcmi-terms/")){
				newannprop = factory.getOWLAnnotationProperty(
						IRI.create(RDFNamespace.DCTERMS.getNamespaceAsString() + oldannprop.getIRI().getFragment()));
			}
			
			if(newannprop!=null) {
				OWLAnnotation newann = factory.getOWLAnnotation(newannprop, annval);
				manager.applyChange(new AddOntologyAnnotation(ont, newann));
				manager.applyChange(new RemoveOntologyAnnotation(ont, oldann));
			}
		}

		// Set the DC prefix
		OWLOntologyFormat oof = manager.getOntologyFormat(ont);
		oof.asPrefixOWLOntologyFormat().setPrefix("dc", RDFNamespace.DCTERMS.getNamespaceAsString());
		manager.setOntologyFormat(ont, oof);
		oof = manager.getOntologyFormat(ont);
		...
		for (OWLAnnotation named : anns) {
			...
			if (named.getProperty().getIRI().equals(SemSimModel.LEGACY_CODE_LOCATION_IRI)) {
				...
				if( ! location.isEmpty()){
					...
					annstoremove.add(named);
					System.out.println("Legacy code at " + location);
				}
			}
