...
public class CompositeAnnotationPanel extends Box implements ActionListener{
	...
	public CompositeAnnotationPanel(int orientation, int ind){
		super(orientation);
		indent = ind;
		this.setBackground(SemGenResource.lightblue);
	...
	public void refreshUI(){
		...
		addentprocpanel.setBackground(SemGenResource.lightblue);
	...
	public void alignAndPaint(){
		...
		for(Component c : getComponents()){
			...
			c.setBackground(SemGenResource.lightblue);
			x = x + 15;
		}
	...
	public void setAddButtonsEnabled(){
		...
		if(ad.thebutton.editable){
			...
			// based on type of physical property specified 
				...
				int type = datastructure.getPropertyType();
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(arg0.getSource() == addentbutton){
			if(datastructure.getPhysicalProperty().getPhysicalPropertyOf()==null){
				datastructure.getPhysicalProperty().setPhysicalPropertyOf(semsimmodel.getCustomPhysicalEntityByName(SemGenResource.unspecifiedName));
			}
			else{
				...
				ents.add(semsimmodel.getCustomPhysicalEntityByName(SemGenResource.unspecifiedName));
		...
		if(arg0.getSource() == addprocbutton){
			if(datastructure.getPhysicalProperty().getPhysicalPropertyOf()==null){
				datastructure.getPhysicalProperty().setPhysicalPropertyOf(semsimmodel.getCustomPhysicalProcessByName(SemGenResource.unspecifiedName));
			}
			refreshUI();
		}
	}
