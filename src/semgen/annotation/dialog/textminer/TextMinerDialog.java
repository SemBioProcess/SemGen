...
public class TextMinerDialog extends JDialog implements PropertyChangeListener, ActionListener{
	...
	public TextMinerDialog() throws FileNotFoundException{		
		...
		loadingbutton = new JButton(SemGenIcon.blankloadingicon);
		...
		sppanel = new JPanel(new BoxLayout(sppanel, BoxLayout.Y_AXIS));
		...
		resultspanel = new JPanel(new BoxLayout(resultspanel,BoxLayout.Y_AXIS));
		...
		Object[] options = { "Collect & Close", "Collect", "Close" };
	...
	public void parse() throws FileNotFoundException{
		...
		try {
			...
			SemGenError.showWebConnectionError(optionPane, "BioPortal Annotator service");
		}
		try {
			...
			if(!annit.hasNext()){resultspanel.add(new JLabel("NCBO annotator did not find any ontology terms"));}
				...
				while(annit.hasNext()){
					...
					String ontacronym = "";
					if(shortid.contains(":")){
						ontacronym = shortid.substring(0,shortid.indexOf(":")).toUpperCase();
					}
					else{ontacronym = "OPB";}
					...
					if(!boxnames.contains(boxname)){
						...
						TextMinerPanel newpanel = new TextMinerPanel(box, 
								"http://rest.bioontology.org/bioportal/ontologies/download/" + concept.getChildText("localOntologyId") 
								+ "?apikey=" + SemSimConstants.BIOPORTAL_API_KEY, bioportalID, uri);
		...
		for(int c=0;c<comps.length;c++){
			if(comps[c] instanceof TextMinerPanel){
				...
				if(panel.box.isSelected()){
					...
					if(panel.bioportalID.equals("1070")){ // If GO
